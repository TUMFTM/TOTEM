function [par_VEH, I] = I_z(par_VEH, m, IP, NR, par_MDT)
%%Berechnung des Trägheitsmoment um die z-Achse
...
r_mid =     1.8/12 * a_mid * ((IP.Radstand)^2 + (IP.Breite)^2) + ...
            a_mid * ((0.5 * IP.Radstand)^2 + (0.5 * IP.Breite)^2);
r_rear =    a_rear * (IP.UeH_v + IP.Radstand + IP.UeH_h * 0.5 - xSP)^2;

I.ST.frame =      m.ST.frame * (r_front + r_mid + r_rear);
I.ST.crash_fr =   m.ST.crash_fr * ((IP.UeH_v * 4.1/2.7 * 0.5 - xSP)^2 + (0.4 * IP.Breite)^2);
I.ST.crash_re =   m.ST.crash_re * ((IP.UeH_v + IP.Radstand + IP.UeH_h * 0.5 - xSP)^2 + (0.4 * IP.Breite)^2);

I.Sum.ST =  I.ST.frame + I.ST.crash_fr + I.ST.crash_re;

%%Chassis
I.CH.ax_re_ml =       m.CH.ax_re_ml * (IP.UeH_v - xSP)^2;
I.CH.susp_re_ml =     m.CH.susp_re_ml * ((IP.UeH_v - xSP)^2 + (0.5 * IP.Breite)^2);
I.CH.ax_fr_mcp =      m.CH.ax_fr_mcp * (IP.UeH_v + IP.Radstand - xSP)^2;
I.CH.susp_fr_mcp =    m.CH.susp_fr_mcp * ((IP.UeH_v + IP.Radstand- xSP)^2 + (0.5 * IP.Breite)^2);

I.CH.steer =              m.CH.steer * (IP.UeH_v * 1.2 - xSP)^2; %%%!!!!      
I.CH.brake_total =        m.CH.brake_total * ((IP.UeH_v + IP.Radstand * 0.4 - xSP)^2 + (0.5 * NR.SpW_avg)^2);
I.CH.wheels_fr_normal =   m.CH.wheels_fr_normal * ((IP.UeH_v - xSP)^2 + (0.5 * NR.SpW_avg)^2);
I.CH.wheels_re_normal =   m.CH.wheels_re_normal * ((IP.UeH_v + IP.Radstand - xSP)^2 + (0.5 * NR.SpW_avg)^2);
...
if IP.electric == 0
    ...
    I.DT.HVBatt =     1/12 * m.DT.HVBatt * (IP.Radstand^2 + IP.Breite^2);
...
if IP.electric == 0
    ...
    I.EE.nv_wiring =  m.EE.nv_wiring * (IP.UeH_v + (IP.Radstand + IP.UeH_h) * 0.5 -xSP)^2;
    I.EE.light_ext =  m.EE.light_ext * ((IP.UeH_v + IP.Radstand + IP.UeH_h) * 0.5 -xSP)^2;
...
%%Exterieur
I.EX.doors =      m.EX.doors * ((IP.UeH_v + IP.Radstand * 5.0/8.65 -xSP)^2 + (0.5 * NR.SpW_avg)^2);
