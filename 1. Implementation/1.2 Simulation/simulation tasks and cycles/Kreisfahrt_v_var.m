...
% Erzeugung der Input Daten für das Vormanöver für Lenkwinkelsprung und Beschleunigung aus stat. Kreisfahrt
...
psip0 = 0;
simtime = 15;                                                               % [s] Simulationsdauer
input_time = (0 : stepsize: simtime)';                                      % [s] Zeitvektor
v0 = 8;                                                                     % [m/s] Startgeschwindigkeit
v_final =15.8;                                                              % [m/s] Endgeschwindigkeit

% Vorgaben als Array: [time, data]
input_r = Radius * ones(length(input_time), 1);                             % quasistat. Kreisfahrt
input_v = interp1([0,simtime], [v0, v_final], input_time);                  % konstante Beschleunigung
input_psip = zeros(length(input_time), 1);                                  % Nutzung im Fahrerregler
input_delta_h =  zeros(length(input_time), 1);                              % kein Lenkwinkel (hier Regelung nach Radius)
