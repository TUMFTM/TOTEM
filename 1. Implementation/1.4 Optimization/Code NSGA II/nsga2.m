...
pop = repmat( struct(...
    ...
    'violSum', 0,...
    'Verbrauch', zeros(1,1),...
    'Bewertungswerte', zeros(1,16),...
    'Bewertungsnoten', zeros(1,16)),...
    [1,popsize]);
...
GenTimer=tic;
[pop, state] = evaluate_using_pareval(opt, pop, state, varargin{:});
fprintf('Evaluating Generation %d / %d took %d minutes %d seconds\n', state.currentGen, opt.maxGen, floor(toc(GenTimer)/60), round(mod(toc(GenTimer),60)));
...
while( ngen < opt.maxGen && STOP_NSGA==0)
    ...
    state.currentGen = ngen;
    fprintf('\n\n************************************************************\n');
    fprintf('*      Current generation %d / %d (Config %d%d)\n', ngen, opt.maxGen,varargin{1,3}.lb(1), varargin{1,3}.lb(2));
    fprintf('************************************************************\n');
    t = [datetime('now')];
    ...
    GenTimer=tic;
    [newpop, state] = evaluate_using_pareval(opt, newpop, state, varargin{:});
    fprintf('Evaluating Generation %d / %d took %d minutes %d seconds\n', state.currentGen, opt.maxGen, floor(toc(GenTimer)/60), round(mod(toc(GenTimer),60)));
    ...
    if( mod(ngen, 5)==0 )
        ...
        save(['./Optimierung/Auswertung/Optimierungsergebnisse/',filename.name_1,filename.name_2,'_partial_result_gen_', num2str(ngen),'_',num2str(t(1)),'_',num2str(t(2)),'_',num2str(t(3)),'_',num2str(t(4)),'_',num2str(t(5)),'_',num2str(round(t(6)))],'result');
    end
    
end


% Abspeichern der Optimierungsergebnisse unter dem Namen 'result'
t=clock; conf = varargin{4};
assignin('base', 'result', result)
if conf == 1
    filename = varargin{3};
    save(['./1.4 Optimization/Results/',filename.name_1,filename.name_2,'_result_',num2str(t(1)),'_',num2str(t(2)),'_',num2str(t(3)),'_',num2str(t(4)),'_',num2str(t(5)),'_',num2str(round(t(6)))],'result');
else
    save(['./1.4 Optimization/Results/result_',num2str(t(1)),'_',num2str(t(2)),'_',num2str(t(3)),'_',num2str(t(4)),'_',num2str(t(5)),'_',num2str(round(t(6)))],'result');
end
